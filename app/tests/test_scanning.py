...
from services.scanning import do_scan_sbom, latest_sbom_ids
...
async def test_latest_sbom_ids_picks_latest_per_scope(db_session):
    ...
    result = await latest_sbom_ids(db_session)
...
async def test_do_scan_sbom_skips_reconcile_when_scan_fails(db_session):
    ...
    with patch("services.scanning.scan_with_grype", new_callable=AsyncMock, return_value=None):
        await do_scan_sbom(db_session, str(latest.id))
...
async def test_do_scan_sbom_reconciles_on_success(db_session):
    ...
    with patch("services.scanning.scan_with_grype", new_callable=AsyncMock, return_value=[]):
        await do_scan_sbom(db_session, str(latest.id))
...
async def test_do_scan_sbom_retires_stale_vulns_on_latest(db_session):
    ...
    with patch(
        "services.scanning.scan_with_grype",
...
async def test_do_scan_sbom_invalid_uuid(db_session):
    await do_scan_sbom(db_session, "not-a-uuid")


@pytest.mark.asyncio
async def test_do_scan_sbom_not_found(db_session):
    await do_scan_sbom(db_session, str(uuid.uuid4()))
